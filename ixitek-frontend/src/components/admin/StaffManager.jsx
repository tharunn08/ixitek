...
import { getStaff, addStaff, deleteStaff, subscribeToStaff } from "../../lib/staffStore.js";
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <p className="text-sm leading-relaxed text-ink-500">
                ...
                in on the login page using the email and password you set here.
              </p>
                ...
                <ul className="mt-2 flex flex-col gap-2">
                  {staff.map((s) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm(`Remove ${s.name}'s access?`)) deleteStaff(s.id);
                        }}
              ...
              <div className="mt-6 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-[11px] leading-relaxed text-amber-800">
                <Icon name="Info" className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                <span>
                  Teammate accounts are saved in this browser only (no backend/server). Add each
                  person's account from the same browser/computer they'll actually sign in from —
                  an account added here won't be visible on a different device.
                </span>
              </div>
            </div>
          </motion.aside>
