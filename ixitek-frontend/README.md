...
with a modern white + blue design system. Product/company data is static (see `src/data/`). Accounts,
authentication, and the contact/career/enquiry forms are backed by a real Node.js/SQLite API — see
`../ixitek-backend` and `ADMIN_SETUP.md` for setup. Set `VITE_API_URL` in `.env` to point this app at
that backend (defaults to `http://localhost:5000`).
